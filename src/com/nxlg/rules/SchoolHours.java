...
public class SchoolHours extends TCcRSWRule {
    ...
    public double calculatePunishValue() {
        int repeatCnt = 0;
        Map<Integer, Integer> map = PrjUtils.CouseCntCount(data, (TCcRSwIndex) tCcRSwIndex);
        ...
        while (entries.hasNext()) {
            ...
            if (entry.getKey() != 0) {
                ...
                //计算并比较周学时
                if (weekhours % 2 != 0) {
                    if ((weekhours + 1) != entry.getValue() * 2)
                        //要被惩罚的课程数 = 课程重复次数 - 每周应该安排的课程数
                        repeatCnt = repeatCnt + Math.abs(entry.getValue() - ((weekhours + 1) / 2));
                } else {
