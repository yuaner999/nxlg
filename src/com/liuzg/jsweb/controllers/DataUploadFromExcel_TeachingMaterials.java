...
/**
 * 上传excel文件，批量导入教材信息的controller
 * Created by zcy on 2017/6/3.
 */

@Controller
@RequestMapping("/TeachingMaterials")
@Scope("prototype")
public class DataUploadFromExcel_TeachingMaterials {
        @Autowired
 ...
 */
         ...
         * @return
         */
        @RequestMapping("/TeachingMaterials_info")
        @ResponseBody
...
//        System.out.println("验证excel文件中数据的完整及唯一性完成");
                ...
                //验证数据是否和数据库中的数据有重复
                if(tableName!=null && tableName.contains("TeachingMaterials")){
                    resultData=primaryKeyValid(list);
                    if(resultData.getStatus()!=0)
                        return resultData;
                }
                //把转换完成的数据交给数据存储处理函数
                List<String > error_list=saveTeachingMaterials(list);
...
//                System.out.println("上传文件检查文件完成");
         ...
         */
        private List<String> saveTeachingMaterials(List<Map<String ,String >> list) throws ParseException {
            //缴费信息列表
            //List<EducatePlan> EducatePlans=new ArrayList<>();
            //错误信息存放集合
            List<String > errorList=new ArrayList<>();
            int i=0;
            List<String> TeachingMaterial =teachingMaterialsService.getKeys(list);
            if(TeachingMaterial.size()>0){
                errorList.add("教材信息已存在，请仔细核对");
            }
            ...
            for(Map<String ,String > map:list){
                ...
                if(i%batch==0){
                    try{
                        ...
                        if(tableName.contains("TeachingMaterials")){
                            result= teachingMaterialsService.saveTeachingMaterials(list);
                        }
                        if(result<=0){
...
//        }
        ...
        private List<ResultData> uqValid(List<Map<String ,String >> list){
            ...
            for(Map<String ,String > m:list){
                if(tableName!=null && tableName.contains("TeachingMaterials")){
                    ...
                    if(value==null || value.equals(""))
                        classNameErr.add("第 "+i+" 行:教材名称为空");
                    value=m.get("press");
                    if(value==null || value.equals(""))
                        classNameErr.add("第 "+i+" 行:出版社为空");
                    value=m.get("edition");
                    if(value==null || value.equals(""))
                        classNameErr.add("第 "+i+" 行:版次为空");
                    value=m.get("booknumber");
                    if(value==null || value.equals(""))
                        classNameErr.add("第 "+i+" 行:书号为空");
                    value=m.get("price");
                    if(value==null || value.equals(""))
                        classNameErr.add("第 "+i+" 行:价格为空");
                }else{
            ...
            if(classNameErr.size()>0){
                ...
                re.sets(1,"教材名称、出版社、版次、书号、价格 数据有错误");
                ...
                if(i%batch!=0){
                    try{
                        ...
                        if(tableName.contains("TeachingMaterials")){
                            ...
                            List<String > keys= teachingMaterialsService.getKeys(list1);
                            temp.addAll(keys);
                            if(temp.size()!=0){
                                resultData.sets(1,"第"+i+"行教材信息已存在");
                                return resultData;
                            }
                            temp.clear();
                        }
                        ...
                        e.getStackTrace();
                        String msg=e.getMessage();
                        resultData.sets(2,"第"+i+"行教材信息已存在，可能原因："+e.getMessage().substring(0,msg.length()>2000?2000:msg.length()));
                        return resultData;
                    }
//                list.clear();
                }
