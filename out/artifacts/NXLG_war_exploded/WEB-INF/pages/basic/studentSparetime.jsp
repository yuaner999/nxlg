...
<div style=" position: absolute;
    ...
    <div style="display: inline-block;color:red;font-size: smaller;margin-left: 190px;margin-top: 57px;">
        <span>注：周一1代表星期一第一大节，依次类推，点击右侧设置可以查看详细的空余节次</span>
    </div>
</div>
<!--表格-->
<div style="width:100%">
...
<script>
    ...
    var app = angular.module('app',[]).controller('ctrl',function  ($scope) {
        ...
        $scope.search = function () {
            loading();//加载
            remotecall("studentSparetime_load",{pageNum:pageNum,pageSize:pageSize,searchStr:searchStr},function (data) {
                closeLoading();//关闭加载层
        ...
        $("#MenuForm").validate({
            submitHandler:function(form){
                loading();
                //验证通过,然后就保存
                var setIds = $("input[name='majorId']:checked").map(function(index,elem) {
                    return $(elem).val();
                }).get();//需要设置的教师Id
                var node=new Array();
                var j=0,k=0;
