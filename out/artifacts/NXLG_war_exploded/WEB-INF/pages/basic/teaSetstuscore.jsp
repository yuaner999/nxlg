...
<div class="title">
    ...
    <table-btn ng-if="show=='2'&& filter=='0'" class="top" ng-click="pass()"><img src="<%=request.getContextPath()%>/images/tablepass.png"/>通过</table-btn>
    <table-btn ng-if="show=='2'&& filter=='0'" class="top" ng-click="nopass()"><img src="<%=request.getContextPath()%>/images/tablereject.png"/>不通过</table-btn>
</div>
<div class="tablebox" ng-if="show=='0'">
    <table class="table">
        ...
        <tr ng-repeat="course in courses">
            ...
            <td><table-btn ng-click="choosecourse(course)">选择</table-btn></td>
        </tr>
        </tbody>
    </table>
</div>
...
<div class="tablebox" id="class" ng-if="show=='1'">
    <table class="table">
        ...
        <tr ng-repeat="class in classes">
            ...
            <td><table-btn ng-click="chooseclass(class)">选择</table-btn></td>
        </tr>
        </tbody>
    </table>
</div>
...
<div class="tablebox" id="stu" ng-show="show=='2'">
    <table class="table">
        <thead>
        <th class="checked" ng-show="filter=='0'"><button><input ng-click="allfn()" id="all" style="width: 100%;" type="checkbox" ng-model="all"/></button></th>
        <th>学号</th>
        <th>学生姓名</th>
        <th>专业</th>
        <th>状态</th>
        ...
        <tr ng-repeat="stu in stus">
            ...
            <td><table-btn ng-click="pass1(stu)" ng-show="filter=='0'||filter=='2'">通过</table-btn>&nbsp;&nbsp;
                <table-btn ng-click="nopass1(stu)" ng-show="filter=='0'||filter=='1'">不通过</table-btn></td>
        </tr>
        </tbody>
    </table>
</div>
...
<script>
    ...
    var app = angular.module('app',[]).controller('ctrl',function  ($scope) {
        ...
        $scope.loadData = function (){
            $scope.show=0;
            loading();//加载
            remotecall("teacher_Course_load",{pageNum:pageNum,pageSize:pageSize},function (data) {
                closeLoading();//关闭加载层
        ...
        $scope.choosecourse= function (course) {
            ...
            loading();//加载
            remotecall("teacher_Course_classList",{courseId:$scope.courseitem.courseId,pageNum:pageNum,pageSize:pageSize},function (data) {
                closeLoading();//关闭加载层
                if(data.result==false){
                    showmsgpc(data.errormessage);
                    return;
        ...
        $scope.chooseclass= function  (tr) {
            ...
            loading();//加载
            remotecall("teacher_Courseclass_stuList",{pageNum:pageNum,pageSize:pageSize,filter:$scope.filter,tc_id:$scope.classitem.tc_id},function (data) {
                closeLoading();//关闭加载层
