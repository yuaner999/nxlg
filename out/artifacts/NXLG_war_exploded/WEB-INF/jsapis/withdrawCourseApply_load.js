...
function main(context){
    ...
    */},context,'');
    ...
    var r=querypagedata(function () {/*
     ...
     and case when (${filter} is not null and ${filter}<>"" and ${filter}="1") then (`stuchoosecourse`.`iscomfirm` = "1" and (`stuchoosecourse`.`scc_status` is null or
     `stuchoosecourse`.`scc_status`="退课拒绝" )) else 1=1 end
     and case when (${filter} is not null and ${filter}<>"" and ${filter}="0") then (`stuchoosecourse`.`iscomfirm` = "1" and `stuchoosecourse`.`scc_status` is not null) else 1=1 end
