...
function main(contexts) {
    ...
    for (var i = 0; i < contexts.passIds.length; i++) {
        ...
        var result = multiexec(con, function () {/*
         update major set checkStatus='已通过' where majorId=${majorId};
         */}, {majorId: majorId});//设置状态为已通过
        if (!result) {//出错回滚
            rollback(con);
            closeconnection(con)
            return false;
        ...
        if (contexts.passIds[i].checkType == "删除") {//如果是删除
            ...
            result = multiexec(con, function () {/*
             ...
             update major set majorId=${relationId},relationId='',`updateMan` = ${realName},
             `updateDate` = now() where majorId=${majorId};
