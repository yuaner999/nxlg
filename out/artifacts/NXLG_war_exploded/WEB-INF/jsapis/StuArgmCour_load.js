...
function main(context) {
    var result = querypagedata(function () {/*
     ...
     WHERE
     userId=${sessionUserID} AND is_now=1 AND iscomfirm=1 AND (scc_status!='退课通过' OR scc_status IS NULL) AND (${thweekday}=0 OR tc_teachodd='无' OR tc_teachodd=${teachodd}) AND (${thweekday}=0 OR (tc_thweek_start<=${thweekday} AND ${thweekday}<=tc_thweek_end))
     */},context,"");
    return result;
}