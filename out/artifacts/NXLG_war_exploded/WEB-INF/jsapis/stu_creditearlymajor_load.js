...
function main(context) {
    ...
    var n= query(function () {/*
     ...
     LEFT JOIN stuchoosecourse ON (stuchoosecourse.`majorId`=majorterracescore.`majorId` AND majorterracescore.`terraceId` = stuchoosecourse.`terraceId` AND student.`studentId`=stuchoosecourse.`studentId`AND stuchoosecourse.`pass`="已通过" AND iscomfirm='1' AND (scc_status!='退课通过' OR ISNULL(scc_status)))
