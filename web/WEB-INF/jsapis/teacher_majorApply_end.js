...
function main(context) {
    ...
    var json="[{院(系)/部："+context.majorCollege+"},{国标专业个数："+context.internationalNum+"},{国标专业代码："
        ...
        +context.level+"},{学制："+context.length+"},{设置年份："+context.settingYear+"},{专业状态："+'停用'+"}]";
    context.json=json;
    if(context.checkStatus=='已通过'){
        var result=multiexec(con,function(){/*
         ...
         ${majorName},${subject},${level},${length},${settingYear},"停用","停用","待审核",${majorId},"否",${sessionUserName},NOW(),${trainingobjects},${introduction})
         */},context);
        if(!result){
            rollback(con);
            closeconnection(con);
