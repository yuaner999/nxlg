...
<hr>
<div class="title">
    <table-btn ng-if="show=='2'" ng-click="previousToIndex()">返回上层</table-btn>
</div>
<!--导航筛选-->
<table-nav>
...
<div class="title">
    ...
    <table-btn ng-click="exportInformation()" ng-if="Var==false"><img src="<%=request.getContextPath()%>/images/icon_download.jpg" />导出到Excel</table-btn>
    <form id="export_fm" method="post" hidden></form>
    <span ng-show="show=='2'" class="span_width">平&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;台：</span>
    <select ng-show="show=='2'" type="text" ng-model="terraceName" name="terraceName" class="forminput" id="terraceName"
    >
        <option value="" selected="selected">--请选择--</option>
        <option ng-repeat="terrace in terraces" value="{{terrace.terraceName}}" ng-bind="terrace.terraceName"></option>
    </select>
    <span ng-show="show=='2'" class="span_width">学&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;期：</span>
    <select ng-show="show=='2'" type="text" ng-model="ep_term" name="ep_term" class="forminput" id="ep_term">
        <option value="" selected="selected">--请选择--</option>
        <option value="第一学期">第一学期</option>
        <option value="第二学期">第二学期</option>
    </select>
    <input ng-show="show=='2'" class="tablesearchbtn" type="text" ng-model="input.keyword" placeholder="请输入课程名称进行查询..." onkeyup="getSearchStr(this)"/>
    <input ng-show="show=='1'" class="tablesearchbtn" type="text" ng-model="input.keywordCourse" placeholder="请输入课程名称进行查询..." onkeyup="getSearchStr2(this)"/>
    <span ng-show="show==2&&Var" class="span_width">审核类别：</span>
    <select type="text" ng-model="checkType" name="checkType" class="forminput" id="checkType" ng-show="show==2&&Var"/>
    <option value="">--请选择--</option>
    <option value="新增">新增</option>
    <option value="修改">修改</option>
    <option value="删除">删除</option>
    </select>
    <%--<span ng-show="show=='2'&&Var" class="span_width">审核状态：</span>--%>
    <%--<select ng-show="show=='2'&&Var" type="text" ng-model="checkStatus" name="checkStatus" class="forminput" id="checkStatus"/>--%>
    <%--<option value="">--请选择--</option>--%>
...
<div class="tablebox" id="allInfo">
    <table class="table" style="table-layout:fixed">
        ...
        <tr ng-repeat="Info in allInfo">
            ...
            <td  ng-if="Var&&tab=='未通过'" ng-bind="Info.ep_refuseReason" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width: 100%"
...
<script>
    ...
    var app = angular.module('app',[]).controller('ctrl',function  ($scope) {
        ...
        $scope.loadData = function (){
            ...
            loading();//加载
            remotecallasync("teacher_educatePlan_load",{filter:filter,pageNum:pageNum,pageSize:pageSize,searchStr:searchStr,college:college,major:major,grade:grade,
            ...
            remotecall("teacher_educatePlan_loadgrade",{grade:grade},function (data) {
                $scope.grades = data;
            },function (data) {
                parent.pMsg("暂无数据");
            });
            ...
            remotecall("teacher_majorApply_load_majorCollege",{college:college},function (data) {
                closeLoading();//关闭加载层
        ...
        $scope.changeCollege=function (str) {
            remotecall("teacher_educatePlan_loadMajor",{ep_college:str,major:major},function (data) {
                $scope.majors = data;
            },function (data) {
                parent.pMsg("暂无数据");
            });
        ...
        $scope.edit= function (tr) {
            add_edit=false;
            $scope.ep_college=tr.ep_college;
            $scope.changeCollege($scope.ep_college);
        ...
        $scope.previousToIndex=function(){
            location.href='educatePlan.form';
        }
        //取消
        ...
        //导出培养计划里的课程信息到Excel中
        $scope.exportInformation = function(){
            var url="../../export/exportEducatePlanMajor.form?searchStr="+searchStr+"&college="+college+"&major="+major+"&grade="+grade;
