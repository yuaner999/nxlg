...
<%--教师最多排课时间--%>
<span style="color:red; margin-left: 20px;">注：周一1代表星期一第一大节，依次类推，点击右侧设置可以查看详细的排课节次和不排课节次</span>
<hr>
<!--导航筛选 此页不需要-->
<!--筛选条件按钮组-->
<div class="title">
    <table-btn class="top" ng-click="edits()"><img src="<%=request.getContextPath()%>/images/tablechange_07.png" />批量设置</table-btn>
    <span class="span_width">任教单位：</span>
    <select type="text" ng-model="teacherCollege" name="teacherCollege" class="queryinput" id="teacherCollege">
        <option value="" selected="selected">--请选择--</option>
        <option value="{{college.wordbookValue}}" ng-repeat="college in teachercolleges" ng-bind="college.wordbookValue"></option>
    </select>
    <span class="span_width">教师工号：</span>
    <input class="tablesearchbtn" type="text" ng-model="teacherNumber" placeholder="请输入教师工号"/>
    <span class="span_width">教师名称：</span>
    <input class="tablesearchbtn" type="text" ng-model="teacherName" placeholder="请输入教师名称"/>
