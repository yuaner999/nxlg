...
<div class="title">
    ...
    <table-btn class="top" ng-click="deletes()"><img src="<%=request.getContextPath()%>/images/tabledelete_07.png"/>批量删除
    </table-btn>
    ...
    <table class="table">
        ...
        <tr ng-repeat="data in datas">
            ...
            <td>
                ...
                &nbsp&nbsp
                <table-btn ng-click="checked(data)">删除</table-btn>
            </td>
        </tr>
        </tbody>
    </table>
    ...
    <form id="MenuForm">
        <div class="row">
            <ul class="col-sm-3 col-xs-3">
                <li style="display: none"><span>排课Id：</span><input type="text" ng-model="arrangecourseitem.acId"
                                                                   name="acId" class="forminput" id="acId"/></li>
                <li><span>学期：</span>
                ...
                <li><span>课程起始周：</span><input type="number" min="0" ng-model="arrangecourseitem.startWeek" name="startWeek"
                                              class="forminput" id="startWeek"/></li>
                <li><span>课程结束周：</span><input type="number" min="0" ng-model="arrangecourseitem.endWeek" name="endWeek"
                                              class="forminput" id="endWeek"/></li>
                <li><span>每周上课天数：</span><input type="number" min="0" ng-model="arrangecourseitem.days" name="days"
                                               class="forminput" id="days"/></li>
                <li><span>上午上课节数：</span><input type="number" min="0" ng-model="arrangecourseitem.lessonsMorning"
                                               name="lessonsMorning" class="forminput" id="lessonsMorning"/></li>
                <li><span>下午上课节数：</span><input type="number" min="0" ng-model="arrangecourseitem.lessonAfternoon"
                                               name="lessonAfternoon" class="forminput" id="lessonAfternoon"/></li>
                <li><span>晚上上课节数：</span><input type="number" min="0" ng-model="arrangecourseitem.lessonNight" name="lessonNight"
                                               class="forminput" id="lessonNight"/></li>
            </ul>
        </div>
...
<script>
    ...
    var app = angular.module('app', []).controller('ctrl', function ($scope) {
        ...
        $scope.search = function () {
            loading();//加载
            remotecall("arrangecoursedateManage_load", {
                pageNum: pageNum,
                pageSize: pageSize,
                searchStr: searchStr
            }, function (data) {
        ...
        $scope.edit = function (tr) {
            ...
            $scope.arrangecourseitem.startWeek=parseInt($scope.arrangecourseitem.startWeek);
            $scope.arrangecourseitem.endWeek=parseInt($scope.arrangecourseitem.endWeek);
            $scope.arrangecourseitem.days=parseInt($scope.arrangecourseitem.days);
            $scope.arrangecourseitem.lessonsMorning=parseInt($scope.arrangecourseitem.lessonsMorning);
            $scope.arrangecourseitem.lessonAfternoon=parseInt($scope.arrangecourseitem.lessonAfternoon);
            $scope.arrangecourseitem.lessonNight=parseInt($scope.arrangecourseitem.lessonNight);
            $('.table-addform').addClass('a-show');
            $('.table-addform').removeClass('a-hide');
            $('table,.title,.pagingbox').hide();
            $('.table-addform').show();
        ...
        $("#MenuForm").validate({
            submitHandler: function (form) {
                ...
                if (parseInt(parames.days) < 1) {
                    parent.pMsg("排课天数/周不能小于1");
                    closeLoading();
                    return;
                }
                if (parseInt(parames.endWeek) < 1) {
                    parent.pMsg("结束周不能小于1");
                    closeLoading();
                    return;
                }
                if (parseInt(parames.startWeek) < 1) {
                    parent.pMsg("起始周不能小于1");
                    closeLoading();
                    return;
                }
                if (parseInt(parames.lessonAfternoon) < 1) {
                    parent.pMsg("下午课节不能小于1");
                    closeLoading();
                    return;
                }
                if (parseInt(parames.lessonNight) < 1) {
                    parent.pMsg("晚间课节不能小于1");
                    closeLoading();
                    return;
                }
                if (parseInt(parames.lessonsMorning) < 1) {
                    parent.pMsg("上午课节不能小于1");
                    closeLoading();
                    return;
                }
                //验证通过,然后就保存
