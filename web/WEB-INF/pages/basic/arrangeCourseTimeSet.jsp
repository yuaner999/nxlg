...
<img src="<%=request.getContextPath()%>/images/location.jpg" style="width:24px; margin-right: 8px;" />当前位置：开始排课时间设置
<hr>
<!--筛选条件按钮组-->
<div class="title">
...
<div class="table-show" style="width: 100%">
    <div class="row" style="width: 100%">
        <ul>
            <li style="display: block"><span>开始排课时间设置</span></li>
            <li><span>开始时间：</span><span ng-bind="paystart"></span></li>
            <li><span>结束时间：</span><span ng-bind="payend"></span></li>
        </ul>
    </div>
</div>
<!--class a-show  是显示动画  如果要隐藏先删除a-show  再添加a-hide Class 等待300毫秒动画时间之后再隐藏-->
<div class="table-addform container-fluid a-show">
    <img src="<%=request.getContextPath()%>/images/location.jpg" style="width:24px; margin-right: 8px;" />当前位置：开始排课时间设置 > 设置
    <hr>
    <form id="Form">
        <div class="row">
            <ul class="col-sm-3 col-xs-3">
                <li style="display: block"><span>开始排课时间设置</span></li>
                <li><span>开始时间：</span><input ng-model="paystart" id="paystart" class="forminput" readonly/></li>
                <li><span>结束时间：</span><input  ng-model="payend" id="payend" class="forminput" readonly/></li>
            </ul>
        </div>
...
<script>
    var app = angular.module('app',[]).controller('ctrl',function  ($scope) {
        ...
        $scope.loadData = function () {
            loading();//加载
            remotecall("basic_loadArrangeTime",'',function (data) {
                if(data!=null&&data!=""){
                    $scope.paystart = data[0].paystart;
                    $scope.payend = data[0].payend;
                }
                else{
                    parent.pMsg("当前未设置排课开始和结束时间");
                }
                closeLoading();//关闭加载层
            },function (data) {
                parent.pMsg("加载时间失败");
        ...
        $("#Form").validate({
            submitHandler:function(form){
                ...
                if(ConvertDateFromString(paystart)>ConvertDateFromString(payend)){
                    ...
                    remotecall("basic_editArrangeCourseTime",{paystart:paystart,payend:payend},function (data) {
                        if(data){
                            closeLoading();//关闭加载层
                            parent.pMsg("修改成功");
                            setTimeout(function () {
